...
AI coding tools re-read your entire codebase on every task. `dagayn` fixes that. It is a fork of `code-review-graph` that keeps the same structural knowledge-graph workflow while adding first-class Terraform support powered by the bundled `treesitter-tf` grammar.
...
```bash
pip install dagayn                                 # or: pipx install dagayn
dagayn install                 # auto-detects and configures all supported platforms
dagayn build                   # parse your codebase, including Terraform
```
...
`dagayn` recognizes `.tf` and `.tfvars` files, extracts Terraform-native block shapes such as `resource`, `data`, `module`, `variable`, `output`, `check`, and `terraform`, and records built-in function calls plus cross-block references for blast-radius analysis.
...
<p align="center">
  <img src="diagrams/diagram9_language_coverage.png" alt="24 languages organized by category: Web, Backend, Systems, Mobile, Scripting, Markdown, plus Jupyter/Databricks notebook support" width="90%" />
</p>

Full Tree-sitter grammar support for functions, classes, imports, call sites, inheritance, and test detection in every language. Includes Zig, PowerShell, Julia, Svelte SFC, and Markdown document support. Plus Jupyter/Databricks notebook parsing (`.ipynb`) with multi-language cell support (Python, R, SQL), and Perl XS files (`.xs`).
...
| **24 languages + notebooks** | Python, TypeScript/TSX, JavaScript, Vue, Svelte, Go, Rust, Java, Scala, C#, Ruby, Kotlin, Swift, PHP, Solidity, C/C++, Dart, R, Perl, Lua, Zig, PowerShell, Julia, Markdown, Jupyter/Databricks (.ipynb) |
