...
uses Accelerate on macOS, system BLAS on Linux, and SIMD/scalar fallbacks
elsewhere. Linux source builds need a BLAS implementation available at build and
run time, such as `libblas` or OpenBLAS. Set
`DAGAYN_EMBEDDING_SEARCH_BACKEND=auto` to fall back to the pure-Python loop when
