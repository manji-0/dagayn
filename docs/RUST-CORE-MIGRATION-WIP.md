...
Language introduction order: Markdown → Terraform → Rust → Python/notebooks → TypeScript/JS/TSX/JSX → Bash → Go → Java → Ruby → C# → PHP, with the remaining Python-owned language extractors moving only after parity is explicit.
