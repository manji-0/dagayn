"""CLI entry point for dagayn.

Usage:
    dagayn install
    dagayn init
    dagayn build [--base BASE]
    dagayn update [--base BASE]
    dagayn watch
    dagayn status
    dagayn serve [--http] [--host ADDR] [--port PORT]
    dagayn visualize
    dagayn wiki
    dagayn detect-changes [--base BASE] [--brief]
    dagayn register <path> [--alias name]
    dagayn unregister <path_or_alias>
    dagayn repos
    dagayn daemon start [--foreground]
    dagayn daemon stop
    dagayn daemon restart [--foreground]
    dagayn daemon status
    dagayn daemon logs [--repo ALIAS] [-f] [-n N]
    dagayn daemon add <path> [--alias NAME]
    dagayn daemon remove <path_or_alias>
"""
...
if sys.version_info < (3, 10):
    print("dagayn requires Python 3.10 or higher.")
