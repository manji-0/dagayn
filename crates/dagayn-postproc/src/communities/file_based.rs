...
pub(crate) fn detect_file_based(
    ...
    let group_at_depth = |depth: usize| -> HashMap<String, Vec<GraphNode>> {
        ...
        for (node, dir_parts) in nodes.iter().zip(&all_dir_parts) {
            ...
            let key = if !remainder.is_empty() {
                remainder.iter().take(depth).cloned().collect::<Vec<_>>().join("/")
            } else {
                node.file_path
                    .replace('\\', "/")
                    .rsplit('/')
    ...
    for depth in 1..=max_depth.max(1) {
        ...
        let qualifying = groups.values().filter(|members| members.len() >= min_size).count();
        best_groups = groups;
        if qualifying >= 10 {
            break;
        }
    ...
    let cohesions = compute_cohesion_batch(
        &pending.iter().map(|(_, _, qns)| qns.clone()).collect::<Vec<_>>(),
        edges,
    );
