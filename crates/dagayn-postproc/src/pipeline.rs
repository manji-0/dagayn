...
use crate::flows::{trace_flows, TraceOptions, DEFAULT_FLOW_MAX_DEPTH, DEFAULT_FLOW_MAX_NODES};
...
pub fn run_post_processing_json(
    ...
    if let Some(count) = record_step(&mut result.warnings, "Bare-name edge resolution (calls)", || {
    ...
    if let Some(count) =
        record_step(&mut result.warnings, "Bare-name edge resolution (inheritance)", || {
    ...
    if manifest_nodes_json != "[]" || manifest_edges_json != "[]" {
        if let Some(nodes_upserted) = record_step(
            &mut result.warnings,
            "Manifest bridge extraction",
            || {
    ...
    if let Some(flow_count) = record_step(&mut result.warnings, "Flow detection", || {
        ...
        let payload = serde_json::to_string(&flows).map_err(GraphError::from)?;
        store.store_flows_json(&payload)
    }) {
        result.flows_detected = Some(flow_count);
    }
    ...
    if let Some(scores) = record_step(
        &mut result.warnings,
...
mod tests {
    ...
    fn run_post_processing_json_returns_step_counters() {
        ...
        assert!(payload.get("hub_scores_code_persisted").is_some());
    }
}
