"""Lua/Luau construct extraction for the CodeParser walker."""
...
def handle_node(
    ...
    return _extract_lua_constructs(
        parser,
        child,
        node_type,
        source,
...
def _extract_lua_constructs(
    ...
    if node_type == "variable_declaration":
        return _handle_lua_variable_declaration(
            parser,
            child,
            source,
            language,
    ...
    if node_type == "function_declaration":
        return _handle_lua_table_function(
            parser,
            child,
            source,
            language,
    ...
    if node_type == "function_call" and not enclosing_func:
        req_target = _lua_get_require_target(child)
...
def _handle_lua_variable_declaration(
    ...
    for expr in expr_list.children:
        if expr.type == "function_call":
            req_target = _lua_get_require_target(expr)
            if req_target is not None:
                resolved = parser._resolve_module_to_file(
                    req_target,
                    file_path,
