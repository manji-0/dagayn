...
* the worker (:func:`dagayn.tools.queue_worker.run_worker`) drains the queue,
  re-running a kind while new work of that kind keeps arriving, and exits
  after the queue has been empty for the idle window. On startup it recovers
  tasks left ``running`` by a worker that died mid-execution
  (:meth:`TaskQueue.requeue_stale`);
