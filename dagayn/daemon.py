"""Multi-repo watch daemon for dagayn.

Reads ``~/.dagayn/watch.toml`` to configure which repositories
to watch, then spawns one ``dagayn watch`` child process per
...
CONFIG_PATH: Path = Path.home() / ".dagayn" / "watch.toml"
PID_PATH: Path = Path.home() / ".dagayn" / "daemon.pid"
STATE_PATH: Path = Path.home() / ".dagayn" / "daemon-state.json"
...
class DaemonConfig:
    ...
    log_dir: Path = field(default_factory=lambda: Path.home() / ".dagayn" / "logs")
...
def load_config(path: Path | None = None) -> DaemonConfig:
    ...
    Raises:
        OSError: Propagated if the TOML config cannot be read.
    """
...
class WatchDaemon:
    ...
    Each watched repository gets a ``dagayn watch`` child process
    ...
    def _start_watcher(self, repo: WatchRepo) -> None:
        """Spawn a child process running ``dagayn watch`` for *repo*."""
        self._config.log_dir.mkdir(parents=True, exist_ok=True)
