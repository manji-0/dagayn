...
def detect_communities(store: GraphStore, min_size: int = 2) -> list[dict[str, Any]]:
    ...
    if callable(native):
        return json.loads(cast(Callable[[int], str], native)(min_size))
...
def refresh_community_stats(store: GraphStore) -> dict[str, int]:
    ...
    ``{"updated": n, "deleted": m}``.
    """
    native = getattr(store, "refresh_community_stats_json", None)
    if callable(native):
...
def incremental_detect_communities(
    ...
    if callable(native):
        return cast(
            Callable[..., int],
            native,
        )(changed_files, min_size, pre_affected_count)
