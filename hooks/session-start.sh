#!/bin/bash
# Checks for the dagayn knowledge graph and outputs
# guidance for Claude Code at the start of every session.

DB_PATH=".dagayn/graph.db"

if [ -f "$DB_PATH" ]; then
    cat <<'INSTRUCTIONS'
[dagayn] Knowledge graph is available.

When answering questions about this codebase, prefer using the dagayn MCP tools before scanning files manually:
...
INSTRUCTIONS
    ...
    echo "[dagayn] No knowledge graph found. Run /dagayn:build-graph to parse this codebase and enable graph-powered queries."
fi
