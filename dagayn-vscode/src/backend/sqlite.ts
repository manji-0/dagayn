/**
 * Read-only SQLite reader for the dagayn database.
 *
...
try {
  ...
  if (isAbiMismatch) {
    console.error(
      "[dagayn] better-sqlite3 ABI mismatch. " +
        "Your VS Code uses a different Node.js version than the one " +
        "this extension was built for. " +
        "Try: cd ~/.vscode/extensions/dagayn-* && npm rebuild better-sqlite3",
    );
  }
  throw err;
}
...
type ReaderState = { status: "open"; db: DatabaseType } | { status: "closed" };

export class SqliteReader {
  private state: ReaderState = { status: "closed" };
  ...
  constructor(dbPath: string) {
    // Open read-only: all writes are performed by the Python backend.
    // WAL pragma requires write access, but busy_timeout is safe on a
    // readonly handle and avoids SQLITE_BUSY when the Python writer holds
    // the WAL lock.
    const db = new Database(dbPath, { readonly: true });
    db.pragma("busy_timeout = 5000");
    this.state = { status: "open", db };
  }
  ...
  getAllFiles(): string[] {
    const rows = this._db()
      .prepare("SELECT DISTINCT file_path FROM nodes WHERE kind = 'File' ORDER BY file_path")
      .all() as FilePathRow[];
  ...
  getNodesByFile(filePath: string): GraphNode[] {
    const rows = this._db()
      .prepare("SELECT * FROM nodes WHERE file_path = ? ORDER BY line_start")
  ...
  searchNodes(query: string, limit: number = 20): GraphNode[] {
    const pattern = `%${query}%`;
    const rows = this._db()
      .prepare("SELECT * FROM nodes WHERE name LIKE ? OR qualified_name LIKE ? LIMIT ?")
  ...
  getStats(): GraphStats {
    ...
    const totalNodes = (db.prepare("SELECT COUNT(*) AS cnt FROM nodes").get() as CountRow).cnt;

    const totalEdges = (db.prepare("SELECT COUNT(*) AS cnt FROM edges").get() as CountRow).cnt;
    ...
    const filesCount = (
      db.prepare("SELECT COUNT(*) AS cnt FROM nodes WHERE kind = 'File'").get() as CountRow
    ).cnt;
    ...
    try {
      embeddingsCount = (db.prepare("SELECT COUNT(*) AS cnt FROM embeddings").get() as CountRow)
        .cnt;
    } catch {
  ...
  getMetadata(key: string): string | undefined {
    const row = this._db().prepare("SELECT value FROM metadata WHERE key = ?").get(key) as
      | MetadataRow
      | undefined;
    return row?.value;
  }
   ...
   */
  getImpactRadius(changedFiles: string[], maxDepth: number = 2): ImpactRadius {
